...
class YtdlpGUI:
    ...
    def on_subtitles_check(self):
        ...
        if self.subtitles_var.get():
            ...
            self.scan_subs_button.config(state='normal')  # 可以常亮，只是无法选择字幕
    ...
    def download_video(self):
        ...
        chosen_sub = self.subtitle_var.get().strip() if download_sub else None
        chosen_sub_format = self.subformat_var.get() if download_sub else None
        ...
        if self.audio_only_var.get():
            ...
            # 如果同时选择下载字幕，这在逻辑上无意义，因为只下载音频一般没视频画面。但用户要求了就尊重逻辑
            ...
            # 用户选择的分辨率
            chosen_res = self.format_var.get().strip()
    ...
    def parse_sub_choice(self, sub_choice_str):
        # sub_choice_str类似 "en (normal)" or "ja (auto)"
